...
# run shellscript with one argument to set seed
# ran with seeds 1, 2, ..., 100
...
ARDP_time <- system.time({
  ...
  run <- jags.model(file.path(outputdir, "DAGAR_ARDP.txt"),
                    data = model.data1,
                    n.chains = 4,
                    n.adapt = 500)
  update(run, 50000)
  samples <- coda.samples(run, variable.names = model.param,
                          n.iter = 50000)
  sim_matrix <- as.matrix(samples)
  name <- paste0("sim", sim_i, "DAGAR_ARDP_samps.RData")
  save(samples, file = file.path(outputdir, "ARDP_DAGAR_samps", name))
  phis <- sim_matrix[,3:60]
