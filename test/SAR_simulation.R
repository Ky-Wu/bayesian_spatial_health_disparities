...
# Random effects assume a CAR model but analyzed under a SAR model.
# Rho (spatial percentage of total variation parameter) prior is a PC prior on [0, 1].
# Alpha (SAR spatial smoothing parameter) is also assumed fixed.
...
lambda_rho <- .0335
# Construct Q_scaled using SAR structure with smoothing parameter alpha = 0.99
W_tilde <- apply(W, MARGIN = 1, function(x) x / sum(x))
temp_eigen <- eigen(W_tilde)
P_W <- temp_eigen$vectors
P_Winv <- solve(P_W)
L <- temp_eigen$values
rm(W_tilde, temp_eigen, temp)
Q_scaled <- scaled_SARQ(P_W, P_Winv, L, 0.99)
...
true_Q_scaled <- Q * scaling_factor
phi_truediffs <- BYM2_StdDiff(matrix(phi, nrow = 1),
                              rho, true_Q_scaled, X, ij_list)
...
delta <- .10
optim_t <- min(c(t_seq[t_FDR <= delta], 1))
optim_t
...
rejection_path[9020:9119, cor(optim_e_vij, e5_vij_order)]
indx <- 9020:9119
...
saveRDS(samps, file.path(getwd(), "output", "US_gibbs_SAR_sim", "samps_alpha_0_99.Rds"))

phi_diffs2 <- readRDS(file.path(getwd(), "output", "US_exact_sample_sim",
                                "phi_diffs.Rds"))
# spatial model with CAR (true) structure
CAR_samps <- readRDS(file.path(getwd(), "output", "US_gibbs_sample_sim",
                                "samps.Rds"))
...
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "vij_order_graph.png"),
       width = 8, height = 5.5, units = "in", sim_vij_order_graph)
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "FDR_sensitivity_plot.png"),
       width = 4.5, height = 6, units = "in", FDR_sensitivity_plot)
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "optim_e_vij_hist.png"),
       width = 6, height = 4.5, units = "in", optim_e_vij_hist)
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "rho_hist.png"),
       width = 6, height = 4.5, units = "in", rho_hist)
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "auc.png"),
...
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "auc_FDR.png"),
       width = 9, height = 6, units = "in", auc_FDR)
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "auc_FDR_vijorder.png"),
...
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "top100_vij_order_graph.png"),
       width = 8, height = 5.5, units = "in", sim_vij_order_graph)

# what if we dont divide by conditional posterior variance?
phi_diffs <- vapply(seq_len(nrow(ij_list)), function(pair_indx) {
...
ggsave(file.path(getwd(), "output", "US_gibbs_SAR_sim", "top100_unstd_vij_order_graph.png"),
       width = 8, height = 5.5, units = "in", sim_vij_order_graph)
