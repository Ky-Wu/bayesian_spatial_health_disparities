...
reps <- 50
sim_metrics <- lapply(seq_len(reps), function(i) {
  print(paste0("Simulation Replicate ", i, "/", reps))
  Y <- replicate(K, rnorm(N, mean = mu + sqrt(sigma2Sp) * phi,
                          sd = sqrt(sigma2NSp)))
  nimble_data <- list(Y = Y)
  base_init <- list(tau2 = 0.5,
                    rho = 0.5)
  ...
  decisions <- logical(nrow(ij_list))
  decisions[optim_e_vij >= optim_t] <- TRUE
  out <- ComputeClassificationMetrics(decisions, true_diff)
  out
})
