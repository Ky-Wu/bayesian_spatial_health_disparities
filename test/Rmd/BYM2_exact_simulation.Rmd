...
Figure 2b: Histogram of the posterior probabilities $h_1(\epsilon_{\text{CE}}), ..., h_K(\epsilon_{\text{CE}})$.
```{r}
...
Figure 4: Rankings of the simulated data posterior probabilities of $h_1(\epsilon), ..., h_K(\epsilon)$ for four different values of $\epsilon$ against the final posterior probabilities $h_1(\epsilon_{\text{CE}}), ..., h_K(\epsilon_{\text{CE}})$ when conditioning on the true value of $\rho$. True posterior differences according to the true values of $\phi$ and $\rho$ are shown in blue.
```{r}
