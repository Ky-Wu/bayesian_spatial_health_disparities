...
Figure 5a: Comparison of ROC curves for simulated data when fixing $\rho$ as a hyperparameter at the true simulation value versus placing a PC prior on
$\rho$ and using Gibbs sampling. The classification performance of both models are
compared using fixed difference threshold $\epsilon_{\text{CE}} = 1.105$. 
...
Figure 5b: Comparison of Bayesian FDR versus true sensitivity curves for simulated data when fixing $\rho$ as a hyperparameter at the true simulation value versus placing a PC prior on
$\rho$ and using Gibbs sampling. The classification performance of both models are
compared using fixed difference threshold $\epsilon_{\text{CE}} = 1.105$. 
```{r}
...
Figure 4: Rankings of the posterior probabilities $v_1(\epsilon), ..., v_K(\epsilon)$ for $\epsilon = 0.368, 0.737, 1.657, 3.314$ against $v_1(\epsilon_{\text{CE}}), ..., v_K(\epsilon_{\text{CE}})$ produced from using simulated data and a PC prior on $\rho$. True posterior differences according to the true simulated values of $\phi$ and $\rho$ are shown in blue. 
